...
contract NFTMarketplace is ERC1155Holder,ReentrancyGuard, Ownable{
    ...
    function listLandNft(address _mintContract, uint256 _price, uint256 _tokenId) public nonReentrant {
        ...
        ERC721(_mintContract).safeTransferFrom(msg.sender, address(this), _tokenId); 
    ...
    function buyLandNft(uint256 listIndex, uint256 price) external payable nonReentrant {
        ...
        require(sellerAmount > 0, "Your value must be greater than zero");
        payable(userLandListings[listCount[listIndex].contractAddress][listCount[listIndex].tokenId].owner).transfer(sellerAmount);
        ERC721(listCount[listIndex].contractAddress).safeTransferFrom(address(this), msg.sender, listCount[listIndex].tokenId);
    ...
    function buyBuildings(uint256 bundleListingId) external payable nonReentrant {
        ...
        require(sellerAmount > 0, "Your value must be greater than zero");
        require(artistFee > 0, "Your value must be greater than zero");
   ...
   /**
    ...
    function stakeNFT(address mintContract, address stakerAddress, uint tokenId) public lockConditions(tokenId)
    {
        ...
        totalLockedNft++;
        ERC721(mintContract).safeTransferFrom(stakerAddress, address(this), tokenId);
    }
    ...
    function unStakeNFT(address mintContract , address stakerAddress, uint tokenId) public unlockConditions(tokenId, stakerAddress) {
        ...
        totalLockedNft--;
        ERC721(mintContract).safeTransferFrom(address(this), stakerAddress, tokenId);
    }
 ...
 /**
    ...
    function userClaimFT(address userAddress, uint tokenId) public {
        ...
        if (lockedNFT[tokenId].withdrawMonth + month < 12) {
            ...
            require((reward * month) > 0, "Your value must be greater than zero");
            ...
            require((reward * remainingMonth) > 0, "Your value must be greater than zero");
    ...
    function adminDepositToken(address adminAddress, uint tokenDeposit) public onlyOwner {
        require(tokenDeposit > 0, "Your value must be greater than zero");
        uint256 balanceBefore = IERC20(tokenAddress).balanceOf(address(this));
        IERC20(tokenAddress).safeTransferFrom(adminAddress, address(this), tokenDeposit);
        uint256 balanceAfter = IERC20(tokenAddress).balanceOf(address(this));
        require(balanceAfter - balanceBefore == tokenDeposit, "Incorrect token amount received due to fee-on-transfer.");
    }

    function adminWithdrawToken(address adminAddress, uint tokenWithdraw) public onlyOwner {
        uint256 balanceBefore = IERC20(tokenAddress).balanceOf(address(this));
        IERC20(tokenAddress).safeTransfer(adminAddress, tokenWithdraw);
        uint256 balanceAfter = IERC20(tokenAddress).balanceOf(address(this));
        require(balanceBefore - balanceAfter == tokenWithdraw, "Incorrect token amount sent due to fee-on-transfer.");
    }
    ...
    */
    function cancelOfferList(uint _auctionListCount) external {
        require(auctionListCount[_auctionListCount].amount > 0, "Your value must be greater than zero");
    ...
    function ClaimNFT(uint _auctionListCount) external payable {
        ...
        require(sellerAmount > 0, "Your value must be greater than zero");
        require(artistAmount > 0, "Your value must be greater than zero");
        require(NftAuction[contractAddress][tokenId].amount > 0, "Your value must be greater than zero");
