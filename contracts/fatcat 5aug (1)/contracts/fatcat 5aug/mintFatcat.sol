...
contract Minting is IERC721, ERC721URIStorage, Ownable{
    // Marketplace Address 
    ...
    // Mapping to track processed token IDs
    ...
    */
    function safeMint(string memory uri,address artist,uint artistFeePerAge,string memory collectionId) public {
        ...
        count[msg.sender]++;
        transferFrom(msg.sender, artist, _tokenIdCounter.current());
        Auction(marketplaceAddress).AuctionOfferList(address(this),_tokenIdCounter.current(),0,msg.sender,artistFeePerAge);
        NFTMetadata[_tokenIdCounter.current()] = NFT(block.timestamp,artist,artistFeePerAge, ownerOf(_tokenIdCounter.current()));
...
function getTokenId(address to) public view returns (MyNft[] memory) {
    ...
    console.log("Count for address:", to, count[to]);  

     // Step 1: Find and remove duplicates
    uint256[] memory uniqueTokenIds = new uint256[](count[to]);
    uint256 uniqueCount = 0;
    bool isDuplicate;  

    for (uint256 i = 0; i < count[to]; i++) {
        uint256 tokenId = TokenId[to][i + 1];
        console.log("Checking Token ID:", tokenId);  // Debugging: log tokenId
        ...
        // If not duplicate, add to uniqueTokenIds
        if (!isDuplicate) {
            uniqueTokenIds[uniqueCount] = tokenId;
            uniqueCount++;
        } else {
            console.log("Duplicate found and skipped:", tokenId);
        }
    }

    // Step 2: Process unique token IDs
    for (uint256 i = 0; i < uniqueCount; i++) {
        uint256 tokenId = uniqueTokenIds[i];
        console.log("Processing Token ID:", tokenId);  // Debugging: log tokenId

        if (ownerOf(tokenId) == to) {
            NFT memory nft = NFTMetadata[tokenId];
            myArray[arrayIndex] = MyNft(
                tokenId,
                nft.mintTime,
                ownerOf(tokenId),
                address(this),
            ...
            arrayIndex++;
            console.log("Token added to array index:", arrayIndex); // Debugging: log array index
        }
    }

    // Resize the array to remove empty elements
    assembly {
        mstore(myArray, arrayIndex)
    }
    ...
    */
    function updateTokenId(address _to,uint _tokenId,address _seller) public {
        ...
        processedTokenIds[_tokenId] = true;
    ...
    */
        ...
        returns (MyNft[] memory)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            myArray[i] = MyNft(currentTokenId,NFTMetadata[currentTokenId].mintTime,ownerOf(currentTokenId),address(this),NFTMetadata[currentTokenId].mintArtist,NFTMetadata[currentTokenId].artistFeePerAge,tokenURI(currentTokenId));
        }
        return myArray;
    }
    /**
