...
contract Marketplace is ReentrancyGuard , Ownable{
    ...
    ListTokenId public listedNftToken;
   ...
   // Mappings
    ...
    mapping(uint => ListTokenId) public auctionListNFT;
    mapping(address => mapping(uint => mapping(address => uint[]))) public LiveBiddingIndices;
    ...
    * the NFT is not already listed for soyale or auction. It sets the sale price and records the listing
    ...
    */
    ...
    function ListNft(address _mintContract,uint256 _price,uint256 _tokenId,address artist,uint artistFeePerAge) public nonReentrant {
        address _owner = ERC721(_mintContract).ownerOf(_tokenId);
        require(!_idToNFT[_mintContract][_tokenId].listed,"Already Listed In Marketplace!");
        require(_owner== msg.sender,"artist value not matched");
    ...
    *              the listing price of the NFT.0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2,1,0xAb8483F64d
    * 
    * Requirements:
    ...
    */
    function NftOffers(uint _auctionListCount,string memory _name, uint _price) external {
        ...
        address _owner = ERC721(contractAddress).ownerOf(_auctionListCount);
        ...
        ListTokenId memory listTokenId = auctionListNFT[_auctionListCount];
        require(listTokenId.newOwner != msg.sender, "You are not eligible for bidding");        
        ...
        require(_owner != msg.sender,"Owner cannot offer");
    ...
    */
    function cancelOfferList(uint _auctionListCount) external {
        ListTokenId memory listTokenId = auctionListNFT[_auctionListCount];
        require(listTokenId.newOwner != msg.sender,"Only Owner Can Cancel!!");
    ...
    */
    function cancelOfferPrice(uint _auctionListIndex, uint _bidcount) external {
        ...
        uint userCount = userBiddingCount[contractAddress][tokenId][msg.sender];
        uint count = BiddingCount[contractAddress][tokenId][msg.sender][_bidcount];
    ...
    * @dev Retrieves the bidding history for a specific NFT listed in the marketplace.
    ...
    function getBiddingHistory(uint _listCount) external view returns (userDetail[] memory) {
        ...
        for (uint i = 0; i < maxBids; i++) {
            if (Bidding[contractAddress][tokenId][i].user != address(0)) {
                actualCount++;
            }
        }

        userDetail[] memory BiddingHistory = new userDetail[](actualCount);
        uint indexCount = 0;
        for (uint i = 0; i < maxBids; i++) {
            if (Bidding[contractAddress][tokenId][i].user != address(0)) {
